import {sortDateIncTasksFunc} from '../modules/modules-test';

test('проверка сотритовки списка задач по дате в сторону увеличения', () => {
    let info = [
        {
            "id": "Time_0",
    ...
    expect(sortDateIncTasksFunc(info)).not.toBe(info); // проверка равны ли ссылки после изменений
    expect(sortDateIncTasksFunc(info)).not.toBe(info);

    expect(sortDateIncTasksFunc(info)).toEqual(info); // проверка на глубокое сравнение
    expect(sortDateIncTasksFunc(info)).toEqual(info);
});