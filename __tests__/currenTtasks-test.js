import {currTasksFunc} from '../modules/modules-test';

test('проверка фильтрации списка задач по признаку "невыполненные" ', () => {
    let info = [
        {
            "id": "Time_0",
    ...
    expect(currTasksFunc(info)).not.toBe(info); // проверка равны ли ссылки после изменений
    expect(currTasksFunc(info)).not.toBe(info);

    expect(currTasksFunc(info)).toEqual(info); // проверка на глубокое сравнение
    expect(currTasksFunc(info)).toEqual(info);
});